...
<div class="container-card">
    <?php
    // Reformer un tableau pour la gestion du trie par nombres de vote
    $idea = array();
    foreach ($App->TabIdea as $index => $objIdea) {
        $voteNumber = sizeof($objIdea->UserVote);
        $idea["idea_" . $index . ""] = $voteNumber;
    }
    arsort($idea);

    // Mapper le tableau $idea avec le $App->TabIdea
    foreach ($idea as $index => $vote) {
        $id = substr($index, strlen('idea_'));
        $objIdea = $App->TabIdea[$id];
    ?>
        <div id="idea<?php echo $id ?>" class="card-idea">
            ...
            <div class="card-padding">
                <a class="link-container" href="idea.php?id=<?php echo $id ?>" class="card-link">Voir l'idée</a>
            </div>
            <?php
            if (isset($_SESSION['id']) && $objIdea->User == $_SESSION['id']) {
            ?>
                <div class="card-padding">
                    <a class="button-edit link-container" href="editIdea.php?id=<?php echo $id ?>" class="editIdea card-link">Editer</a>
                    <a class="button-edit link-container" href="removeIdea.php?id=<?php echo $id ?>" class="removeIdea card-link">Retirer</a>
                </div>
            <?php
            }
            ?>
...
<script>
    ...
    // Permet de récupérer via un cookie et le $App->TabIdea->UserVote[] les info du vote de l'idée
                ...
                jsondata.TabIdea.forEach(function(value, i) {
                    if (value.UserVote) {
                        ...
                        value.UserVote.forEach(e => {
                            if (e[0] == getCookie("id")) {
                                // formatage et affichage du svg correspondant en couleur
                                let valueLike = e[1];
